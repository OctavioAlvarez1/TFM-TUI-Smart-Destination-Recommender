...
Conversational AI endpoint powered by a Retrieval-Augmented Generation (RAG) pipeline. Accepts a user message and optional conversation history, retrieves relevant destination context from a FAISS index, and returns a reply generated by `gpt-4o-mini`.

The index is lazy-initialized on the first call (building embeddings from `destinations.csv`, `sustainability_scores.csv`, and `congestion_scores.csv`). If `OPENAI_API_KEY` is not set, the endpoint returns a friendly fallback message — it never raises an exception or returns a 422.

**Request Body**

```json
{
  "message": "Which destinations are best for a nature lover in August?",
  "history": [
    { "role": "user", "content": "Tell me about sustainable destinations." },
    { "role": "assistant", "content": "Horizon monitors 20 destinations in Spain..." }
  ]
}
```
...
| `history` | array | No | Prior turns — each object must have `"role"` (`"user"` or `"assistant"`) and `"content"` (string) |

**Response — 200 OK**

```json
{ "reply": "For nature lovers in August, Picos de Europa (D019) stands out with a sustainability score of 91 and low August congestion..." }
```

| Field | Type | Description |
|---|---|---|
| `reply` | string | The assistant's response, grounded in destination data |

The endpoint always returns 200. If `OPENAI_API_KEY` is missing the reply will be a fallback message explaining the feature requires an API key.
...
curl -X POST http://localhost:8000/chat \
  ...
  -d '{"message": "Which destinations have the lowest congestion in July?", "history": []}'
```
